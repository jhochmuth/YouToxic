...
def get_tweet_predictions(username, num_tweets, types, limit_date, start_date, end_date, pipeline):
    ...
    if not validate_username(username):
        return html.Div('Error: Could not find Twitter account associated with that name.',
                        style={'color': 'rgb(255, 0, 0'})
    ...
    if not tweets:
        return html.Div('Error: Twitter account found, but no tweets are associated with that account.',
                        style={'color': 'rgb(255, 0, 0'})
    ...
    if 'Obscenity' in types:
        preds['obscene'], judgements['obscene'] = pipeline.predict_obscenity_multiple(texts)
    if 'Prejudice' in types:
        preds['prejudice'], judgements['prejudice'] = pipeline.predict_prejudice_multiple(texts)
    ...
    judgement_columns = [{'name': [t, 'Judgement'], 'id': '{}_judgement'.format(t)} for t in types]
    pred_columns = ([{'name': [t, 'Prediction'], 'id': '{}_pred'.format(t)} for t in types])
